...
<header>
    ...
    <div class="responsive-header">
        ...
        // Function to generate form for each area
        function generateAreaForm($areaType, $areaName)
        {
            echo "<form method='post' action='mostrar_mesas.php'>";
            echo "<input type='hidden' name='area' value='$areaType'>";
            echo "<input type='submit' name='$areaType' value='$areaName' class='secciones-secund'>";
            echo "</form>";
        }

        try {
            // Query to get areas of type 'terraza'
            $sqlTerraza = "SELECT * FROM tbl_sala WHERE tipo_sala = 'terraza'";
            $stmtTerraza = $conn->query($sqlTerraza);

            // Display forms for each 'terraza' area
            echo "<div class='flex terraza_opciones salas'>";
            while ($filaTerraza = $stmtTerraza->fetch(PDO::FETCH_ASSOC)) {
                $nombreTerraza = $filaTerraza['nombre'];
                generateAreaForm('terraza', $nombreTerraza);
            }
            echo "</div>";

            // Query to get areas of type 'comedor'
            $sqlComedor = "SELECT * FROM tbl_sala WHERE tipo_sala = 'comedor'";
            $stmtComedor = $conn->query($sqlComedor);

            // Display forms for each 'comedor' area
            echo "<div class='flex comedor_opciones salas'>";
            while ($filaComedor = $stmtComedor->fetch(PDO::FETCH_ASSOC)) {
                $nombreComedor = $filaComedor['nombre'];
                generateAreaForm('comedor', $nombreComedor);
            }
            echo "</div>";

            // Query to get areas of type 'privada'
            $sqlSalaPrivada = "SELECT * FROM tbl_sala WHERE tipo_sala = 'privada'";
            $stmtSalaPrivada = $conn->query($sqlSalaPrivada);

            // Display forms for each 'privada' area
            echo "<div class='flex sala_opciones salas'>";
            while ($filaSalaPrivada = $stmtSalaPrivada->fetch(PDO::FETCH_ASSOC)) {
                $nombreSalaPrivada = $filaSalaPrivada['nombre'];
                generateAreaForm('sala_privada', $nombreSalaPrivada);
            }
            echo "</div>";
...
function filtrarMesasPorFecha($conn, $fechaFiltro)
{
    try {
        ...
        $stmtFiltroFecha = $conn->prepare($sqlFiltroFecha);
...
<body>
    <div class="row flex">
        <div id="restaurante">
            ...
            // Asumiendo que el área se pasa a través de $_GET['table']
            $selectedTable = isset($_GET['table']) ? $_GET['table'] : '';

            // Llamamos a la función mostrarMesas según el área seleccionada
            if ($selectedArea === 'terraza') {
                if ($selectedTable === 'terraza_1' || $selectedTable === 'terraza_2' || $selectedTable === 'terraza_3' || $selectedTable === 'terraza_4') {
                    mostrarMesas('terraza', $conn);
                }
            } elseif ($selectedArea === 'comedor') {
                if ($selectedTable === 'comedor_1' || $selectedTable === 'comedor_2' || $selectedTable === 'comedor_3') {
                    mostrarMesas('comedor_1', $conn);
                }
            } elseif ($selectedArea === 'sala') {
                if ($selectedTable === 'sala_privada_1' || $selectedTable === 'sala_privada_2' || $selectedTable === 'sala_privada_3' || $selectedTable === 'sala_privada_4') {
                    mostrarMesas('sala_privada', $conn);
                }
            } else {
                // Manejar área no válida o no seleccionada
                echo "Área no válida o no seleccionada";
            }
            ?>
